...
def get_user_tweets(input_string):
	...
	if input_string in CACHE_DICTION:
		...
		search_results = api.user_timeline(input_string, count = 50)
...
for i in umich_tweets:
	for j in i['entities']['user_mentions']:
		user_idlist.append(j['id_str'])
	for j in i['entities']['user_mentions']:
		screen_namelist.append(j['screen_name'])

for j in screen_namelist:
	nums = api.get_user(screen_name = j)
	num_favslist.append(nums['favourites_count'])
	descriptionlist.append(nums['description'])


usertups = list(zip(user_idlist, screen_namelist, num_favslist, descriptionlist))
...
for i in umich_tweets:
	tweet_idlist.append(i['id_str'])
	textlist.append(i['text'])
	###
	time_postedlist.append(i['created_at'])
